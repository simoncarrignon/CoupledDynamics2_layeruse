...
for(t in 2:time){
  ...
  if(t==2){
    ...
    current<-concern_timestep3(pop_info=pop_info,net_b=info_mat,net_d=dis_mat,belief=start[[1]],concern=start[[2]],inf=inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=p_infs[s])
  ...
  if(t>2){
    ...
    current<-concern_timestep3(pop_info=pop_info,net_b=info_mat,net_d=dis_mat,belief=current[[1]],
                              ...
                              l_inf_o,l_hea,l_hea_o,p_inf=p_infs[s])
...
saveRDS(OUT, file.path(path2,paste0("Cnets",params1$NetSelect[nt],"mods",md,"d_eff",r,"p",p_infs[s],".RDS")))
