...
concern_timestep1<-function(pop_info,net_b,net_d,belief,concern,inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=1){
  ...
  for(i in (pop_info$n_Ac+pop_info$n_Bc+1):(pop_info$pop)){
    ...
    #t_con<-t_con[t_con>(pop_info$n_Ac+pop_info$n_Bc)]
    if(length(t_con)>0){
      t_inf<-rbinom(n=1,size=sum(inf[t_con,2]),prob=p_inf)
...
concern_timestep2<-function(pop_info,net_b,net_d,belief,concern,inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=1){
...
concern_timestep3<-function(pop_info,net_b,net_d,belief,concern,inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=1){
...
concern_timestep4<-function(pop_info,net_b,net_d,belief,concern,inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=1){
  ...
  #Calculate changes in belief caused infection status of network connections
  net4<-matrix(0,nr=nrow(net1),nc=ncol(net1))
  for(i in (pop_info$n_Ac+pop_info$n_Bc+1):(pop_info$pop)){
    t_con<-which(net4[i,]==1)
    #t_con<-t_con[t_con>(pop_info$n_Ac+pop_info$n_Bc)]
    if(length(t_con)>0){
